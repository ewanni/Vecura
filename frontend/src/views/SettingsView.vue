...
import { PickFolder } from '../../wailsjs/go/api/App.js'
...
// ---- Persistence ----------------------------------------------------------
// Debounced write of the connection/selection UI state to the backend so it
...
onMounted(async () => {
  try {
    ...
    // Never leave the panel empty: default to the first preset, or to
    // OpenRouter when a system/env key is available so the key shows.
    ...
    // Restore saved providers/values FIRST so onProvider can re-apply the
    // user's connection instead of clobbering it with the preset default.
